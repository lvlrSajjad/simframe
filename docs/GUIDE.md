# simframe — the full guide

This is the long version: what simframe does under the hood, what was
measured, and why each decision went the way it did. The
[README](../README.md) is the short version — install, first run, the tool
list. Everything below used to live there and moved here so a visitor can
install in ten seconds and read this when they want to understand it.

The measurements this document quotes are in [`BENCHMARKS.md`](BENCHMARKS.md)
with N, median and p95; what we expected before each measurement is in
[`EXPERIMENTS.md`](EXPERIMENTS.md); the judgements that changed the plan are
in [`DECISIONS.md`](DECISIONS.md); open and closed defects are in
[`DEFERRED.md`](DEFERRED.md).

## Why an agent driving a simulator is slow
...
| | |
| --- | --- |
| [`BENCHMARKS.md`](BENCHMARKS.md) | the record: N, median, p95, machine, every round |
| [`EXPERIMENTS.md`](EXPERIMENTS.md) | what we believed *before* measuring, and the fourteen times it was wrong |
| [`DEFERRED.md`](DEFERRED.md) | every known defect, open or closed, with the evidence |
| [`ARTICLE.md`](ARTICLE.md) | the argument the whole thing adds up to |
...
- **A warm transition graph removes ~27% of an agent's model round trips** —
  ...
  tool here carries it yet. ([§14](EXPERIMENTS.md))
...
[`BENCHMARKS.md`](BENCHMARKS.md); the judgements, including a phase
cancelled by its own measurement, are in [`DECISIONS.md`](DECISIONS.md).

**And the measurements that changed our minds** are in
[`EXPERIMENTS.md`](EXPERIMENTS.md), with what we expected beforehand
...
| Accessibility tree | **not available (OCR + CV only)** | `uiautomator dump` costs **2,012 ms** a read, against 45 ms for the iOS tree. See [`DEFERRED.md`](DEFERRED.md) |
| A launch confirmed to have reached the front | **not available (the launch is not checked)** | iOS compares the pid `simctl launch` printed against the pid the device reports as frontmost, in **2–5 ms**. Nothing here reports either; `am start` fronts synchronously, which is why it has not bitten — but that is not a check. See [`DEFERRED.md`](DEFERRED.md) |
...
at all. See [`BENCHMARKS.md`](BENCHMARKS.md).
...
working in [`EXPERIMENTS.md`](EXPERIMENTS.md) §14.
...
in [`PRIVATE_API.md`](PRIVATE_API.md) with the evidence behind it, so
...
[`DECISIONS.md`](DECISIONS.md) is the register of judgements that
...
[`BENCHMARKS.md`](BENCHMARKS.md) and the open work in
[`DEFERRED.md`](DEFERRED.md).
